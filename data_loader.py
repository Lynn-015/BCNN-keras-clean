...
class DataLoader(object):
    ...
    def __init__(self, input_size=(224, 224, 3), datapath=None, savepath=None, npypath=None):
        ...
        if npypath != None:
            # load npy file
            self.trainx = np.load(os.path.join(npypath, 'trainx.npy'))
            self.trainy = np.load(os.path.join(npypath, 'trainy.npy'))
            self.testx = np.load(os.path.join(npypath, 'testx.npy'))
            self.testy = np.load(os.path.join(npypath, 'testy.npy')) 

        if savepath != None:
            # save data as npy file
            np.save(os.path.join(savepath, 'trainx.npy'), self.trainx)
            np.save(os.path.join(savepath, 'trainy.npy'), self.trainy)
            np.save(os.path.join(savepath, 'testx.npy'), self.testx)
            np.save(os.path.join(savepath, 'testy.npy'), self.testy)
...
if __name__ == "__main__":
    loader = DataLoader(datapath='d:/data_all/front100', savepath='./data')